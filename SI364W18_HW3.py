...
class TweetForm(FlaskForm):
    ...
    username = StringField('Enter the username of the twitter user (no "@"!):', validators = [Required(), Length(1, 64), username_check])
    display_name = StringField('Enter the display name for the twitter user (must be at least 2 words):', validators = [Required(), display_check])
...
def index():
    ...
    if form.validate_on_submit():
        text_s = form.text.data
        d_name = form.display_name.data
    ...
    ## Or if there is not, then create one and add it to the database
        ...
        if user is None:
            user_b = User(username = usern, display_name = d_name)
    ...
    # And redirect to the list of all tweets
        ...
        elif Tweet.query.filter_by(user_id = user.id, text = text_s).first() != None:
    ...
    # Redirect to the index page
        ...
        tweet_a = Tweet(text = text_s, user_id = User.query.filter_by(username=usern).first().id)
        db.session.add(tweet_a)
        db.session.commit()
...
def see_all_users():
    ...
    return render_template('all_users.html', users = all_users) # Replace with code
...
def long_tweet():
    ...
    for t in all_tweets:
        text = t.text
        d[Tweet.query.filter_by(text = text).first().user_id] = text
    sort1 = sorted(d.items(), key = lambda x: x[1])
    longest = sorted(sort1, key = lambda x:len(x[1])-x[1].count(' '), reverse = True)[0]
    d_name = User.query.filter_by(id = longest[0]).first().display_name
    u_name = User.query.filter_by(id = longest[0]).first().username
    return render_template('longest_tweet.html', tweet = longest, u_name = u_name, d_name = d_name)
# NOTE:
